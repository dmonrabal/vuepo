      ...
      <v-dialog v-model="dialogDelete" max-width="500px">
        <v-card>
          <v-card-title class="text-h5"
            >¿Estás seguro de eliminar este elemento test?</v-card-title
          >
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    async reloadCharts() {
      ...
      await Promise.all(
        charts.map(async (chart) => {
          await chart.series.map(async (serie) => {
            //serie.data = [];
            ...
            // TODO check last serie date
            // If new date is lastest, then update firestore
            if (dta.series && dta.series.length > 0) {
              // Only check first serie
              console.log("[INFO] - Data Found");
      ...
      if (charts.length > 0 && newData) {
        await this.uploadChartList();
      } else {
        console.log('[NO DATA!!]:  size chart: ', charts.length, ', newData: ', newData);
      }
    },
    ...
    async reloadCharts2() {
        ...
        chart.series(async (serie) => {
           ...
           let params = ['xytVxvDJSD7Qqse1VOKdd'];
            ...
            if (dta.series && dta.series.length > 0) {
              ...
              const ds = dta.series[0];
              newData = true;
              console.log('Value New Data: ', newData);
              serie.data = [];
              ds.samples.forEach((s) => {
                serie.data.push({
                  x: utils.formatDate(new Date(s.date)),
      ...
      console.log('Value New Data END: ', newData);
      if (charts.length > 0 && newData) {
        await this.uploadChartList();
      } else {
        console.log('[NO DATA!!]:  size chart: ', charts.length, ', newData: ', newData);
  ...
  async created() {
    ...
    const charts = await this.reloadCharts();
